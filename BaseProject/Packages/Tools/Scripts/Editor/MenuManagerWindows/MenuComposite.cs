...
namespace Base.ToolPackage.Editor.MenuManagerWindows
{
    ...
    internal static class MenuComposite
    {
        ...
        private static readonly EMenuEntryKind[] Kinds =
        {
            EMenuEntryKind.MenuItem,
            EMenuEntryKind.CreateAsset
        };
        ...
        public static void Sync(IReadOnlyDictionary<string, ResolvedMenu> resolved)
        {
            ...
            foreach (EMenuEntryKind kind in Kinds)
                changed |= MenuTree.RemoveEntries(Overlay.RootFor(kind), removeById: id => packageIds.Contains(id));
            ...
            foreach (KeyValuePair<string, ResolvedMenu> pair in resolved)
            {
                ...
                List<MenuNode> target = locked
                    ? Overlay.RootFor(pair.Value.Kind)
                    : Registry.RootFor(pair.Value.Kind);
