...
namespace Base.ToolPackage.Editor.PackageDependencies
{
    ...
    internal static class PackageDefaultsWriter
    {
            ...
            IReadOnlyList<PackageDependencyInfo> packages)
        {
            ...
            {
                builder.Append("        private const string ").Append(Identifier(package.FolderName))
                    .Append(" = \"").Append(package.DisplayName).Append("\";").Append(LineBreak);
            }
        }
        ...
        private static void AppendEntries(StringBuilder builder, IReadOnlyList<PackageDependencyInfo> packages)
        {
            for (int i = 0; i < packages.Count; i++)
            {
                ...
                builder.Append(Indent).Append("new PackageEntry(").Append(Identifier(package.FolderName))
                    .Append(", $\"{BaseUrl}").Append(package.FolderName).Append("\"");
