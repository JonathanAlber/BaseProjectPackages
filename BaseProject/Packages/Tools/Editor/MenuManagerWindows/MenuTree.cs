    ...
    internal static class MenuTree
    {
            ...
            HashSet<string> known)
        {
            ...
            foreach (MenuNode node in nodes)
            {
                ...
                if (match.Kind == EMenuEntryKind.CreateAsset
                    && string.IsNullOrWhiteSpace(entry.CreateFileName))
                {
        ...
        internal static bool AutoGroup(List<MenuNode> root, IReadOnlyDictionary<string, ResolvedMenu> resolved)
        {
            ...
            foreach ((MenuEntryNode node, ResolvedMenu match) in movable)
            {
                ...
                if (match.Kind == EMenuEntryKind.CreateAsset
                    && !string.IsNullOrWhiteSpace(match.DefaultFileName))
        ...
        internal static bool PruneNulls(List<MenuNode> nodes)
        {
            ...
            for (int i = nodes.Count - 1; i >= 0; i--)
            {
                if (nodes[i] is MenuGroupNode group)
                {
                    changed |= PruneNulls(group.Children);
                    continue;
                }

                if (nodes[i] is MenuEntryNode entryNode
