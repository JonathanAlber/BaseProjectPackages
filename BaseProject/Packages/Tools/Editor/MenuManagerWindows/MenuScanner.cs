...
namespace Base.ToolPackage.Editor.MenuManagerWindows
{
    ...
    {
        private const string CheckedRole = "Checked";
        ...
        private static void ScanMenuItems(Dictionary<string, ResolvedMenu> result)
        {
            foreach (MethodInfo method in TypeCache.GetMethodsWithAttribute<DynamicMenuItemAttribute>())
            {
                ...
                Func<bool> validate = CreateBoolMethod(owner, attribute.ValidateMethod, ValidateRole);
                Func<bool> isChecked = CreateBoolMethod(owner, attribute.CheckedMethod, CheckedRole);

                result[id] = ResolvedMenu.MenuItem(defaultPath, execute, validate, isChecked, owner);
            }
        }
        ...
        private static Func<bool> CreateBoolMethod(Type owner, string methodName, string role)
        {
            ...
            {
                CustomLogger.LogWarning($"{role} method '{methodName}' on {owner.Name} must be a static "
