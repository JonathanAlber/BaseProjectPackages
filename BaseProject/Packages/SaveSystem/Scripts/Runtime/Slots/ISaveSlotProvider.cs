...
{
    /// <summary>
    /// Owns slot bookkeeping for one slot model: which slots exist, where the next save is written and
    /// any post-save policy such as pruning. Callers express intent (a selected slot, or none) and the
    /// provider resolves it into a concrete id, so switching the model changes save behavior without
    /// touching the caller.
    /// </summary>
    public interface ISaveSlotProvider
    {
        ...
        /// <summary>
        /// Resolves the id the next save should write to. <paramref name="selectedSlotId"/> is the slot
        /// the player currently has selected, or <c>null</c> for no selection.
        ...
        /// <summary>Runs any post-save policy for this model, for example deleting the oldest saves.</summary>
