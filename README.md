# BaseProjectPackages

My personal collection of base Unity packages, the stack I use myself for every project. Built up over years of past projects (especially [Laskards](https://github.com/Kirschkernweitwurf/Laskards), source also on my GitHub) and continuously updated.

## What's inside

- **ServiceLocator** - clean dependency access without singletons everywhere
- **EventBus** - decoupled event communication
- **Custom Tweening Library** - lightweight tween system
- **Scene Loading** - smooth scene transitions
- **Menu Management** - straightforward UI menu handling
- **Several Tools and Utilities** - helpers, extensions, attributes
- ...and much more!

## Installation

The easiest way is to use the [BaseProjectPackageInstaller](https://github.com/Kirschkernweitwurf/BasePackageInstaller), one click and you're done.

Or install individual packages manually via the Unity Package Manager using a Git URL like:

```
https://github.com/Kirschkernweitwurf/BaseProjectPackages.git?path=BaseProject/Packages/<PackageName>
```

Replace `<PackageName>` with one of: `Tools`, `Attributes`, `Core`, `UI`, `Utility`.

## Why?

I made this to have a solid, consistent base for all my Unity projects. It saves me from rewriting the same systems over and over. If it's useful to you too, great, go ahead and use it.

## Updates

This repo gets updated whenever I improve something in one of my projects. Expect changes over time.
