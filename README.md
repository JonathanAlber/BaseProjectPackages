...
Sixteen packages, Unity 6. Everything below is in here and in use.
...
| Audio | `Audio` | The audio manager, containers and pooled sources | Services |
| Content | `Content` | The prefabs and assets the other packages are wired together with | Audio, Controller Support, Core Debug, Save System, Settings System, UI |
| Controller Support | `ControllerSupport` | Gamepad navigation, input glyphs, haptics | Core |
| Core | `Core` | Menus, scenes, timers, state machines, input, pooling | Tweening |
| Core Debug | `CoreDebug` | The in-game debug menu, cheat console and debug drawing | Core, Tweening |
...
- **Audio** with pooled sources and container assets, in its own package so a silent project skips it
- **Menu framework** with identifier assets and open, close and back handling
- **In-game debug menu** with a cheat console and a log console, plus `DebugDraw` shapes that also render in a player, in its own package so a shipping build leaves it out
